...
// Find the export const nepalData block and replace its sectionD explicitly
const nepalBlockRegex = /(export const nepalData: CountryDetailData = \{[\s\S]*?)(sectionD:\s*\{[\s\S]*?\}\s*,)(\s*sources)/;

mockDataStr = mockDataStr.replace(nepalBlockRegex, `$1${newSectionD.trimStart()}\n  $3`);

fs.writeFileSync('src/components/dashboard/MockData.ts', mockDataStr);

console.log('Fixed regex logic, successfully injected Nepal sectionD insights.');
